...
# `scripts/run_fast_tests.py` for the rationale + when to fall back
# to the full `make test`.
test-fast:
	PYTHONPATH=apps/api PLANMYAGENTS_LOAD_PROMOTED_PROVIDERS_FROM_DB=false PLANMYAGENTS_PROMOTED_PROVIDER_STORE_URL= $(PYTHON) scripts/run_fast_tests.py
...
discovery-smoke:
	PYTHONPATH=apps/api $(PYTHON) scripts/run_discovery_upkeep.py --store .planmyagents_runs/discovery-store.sqlite
	PYTHONPATH=apps/api $(PYTHON) scripts/run_discovery.py --query "find flight booking agents" --store .planmyagents_runs/discovery-store.sqlite --exclude-stale --stale-after-days 30 --limit 5 --no-persist >/dev/null

discovery-research-smoke:
	PYTHONPATH=apps/api $(PYTHON) scripts/run_discovery_research.py --store .planmyagents_runs/discovery-store.sqlite --no-github-live --limit 25 >/dev/null

generalization-eval:
	PYTHONPATH=apps/api $(PYTHON) scripts/run_generalization_eval.py >/dev/null

candidate-verify-smoke:
	PYTHONPATH=apps/api $(PYTHON) scripts/run_candidate_verification.py --store .planmyagents_runs/discovery-store.sqlite --limit 3 --dry-run >/dev/null
...
benchmark-schedule:
	PYTHONPATH=apps/api $(PYTHON) scripts/run_benchmark_scheduler.py \
...
docs-extractor:
	PYTHONPATH=apps/api $(PYTHON) scripts/run_docs_extractor.py \
	  --store .planmyagents_runs/discovery-store.sqlite --limit 5

audit-ranking-sources:
	PYTHONPATH=apps/api $(PYTHON) scripts/audit_ranking_sources.py \
	  --benchmark-store .planmyagents_runs/benchmark-store.json
...
discovery-research:
	@set -a; . ./.env 2>/dev/null || true; set +a; \
	  PYTHONPATH=apps/api $(PYTHON) scripts/run_discovery_research.py \
...
benchmark-credibility-report:
	PYTHONPATH=apps/api $(PYTHON) scripts/benchmark_credibility_report.py \
	  --discovery-store .planmyagents_runs/discovery-store.sqlite \
	  --benchmark-store .planmyagents_runs/benchmark-store.json \
	  --also-json

migrate:
	PYTHONPATH=apps/api $(PYTHON) scripts/apply_migrations.py
...
migrate-apis-without-agents:
	PYTHONPATH=apps/api $(PYTHON) scripts/migrate_apis_without_agents.py
...
dev-bench-report:
	PYTHONPATH=apps/api $(PYTHON) scripts/planmyagents_bench.py run --capability email_verification --provider mock-email --output .planmyagents_runs/email_mock.json
	PYTHONPATH=apps/api $(PYTHON) scripts/planmyagents_bench.py report --input .planmyagents_runs/email_mock.json --output .planmyagents_runs/email_mock.md

hunter-report:
	PYTHONPATH=apps/api $(PYTHON) scripts/planmyagents_bench.py run --capability email_verification --provider hunter --output .planmyagents_runs/hunter.json
	PYTHONPATH=apps/api $(PYTHON) scripts/planmyagents_bench.py report --input .planmyagents_runs/hunter.json --output .planmyagents_runs/hunter.md

demo:
	PYTHONPATH=apps/api $(PYTHON) scripts/serve_demo.py
...
env-check:
	PYTHONPATH=apps/api $(PYTHON) scripts/env_check.py
...
outreach-validate-urls:
	$(PYTHON) scripts/validate_outreach_urls.py

outreach-validate-urls-local:
	$(PYTHON) scripts/validate_outreach_urls.py --base-url http://127.0.0.1:3000
...
# `scripts/backfill_phase5_cells.py` — the only writer that bypasses
# the live scheduler. See
# `apps/api/planmyagents_api/benchmark/rebuild.py` for full rationale.
rebuild-rankings:
	@set -a; . ./.env 2>/dev/null || true; set +a; \
	  PYTHONPATH=apps/api $(PYTHON) scripts/rebuild_agent_rankings.py
...
upkeep:
	./scripts/upkeep_loop.sh

discovery-refresh:
	@set -a; . ./.env 2>/dev/null || true; set +a; \
	  PYTHONPATH=apps/api $(PYTHON) scripts/run_discovery_upkeep.py \
	    --store "$${PLANMYAGENTS_DISCOVERY_STORE_URL:-.planmyagents_runs/discovery-store.sqlite}"

verify-candidates:
	@set -a; . ./.env 2>/dev/null || true; set +a; \
	  PYTHONPATH=apps/api $(PYTHON) scripts/run_candidate_verification.py \
	    --store "$${PLANMYAGENTS_DISCOVERY_STORE_URL:-.planmyagents_runs/discovery-store.sqlite}" \
...
embed-candidates:
	@set -a; . ./.env 2>/dev/null || true; set +a; \
	  PYTHONPATH=apps/api $(PYTHON) scripts/embed_discovery_candidates.py \
...
recipe-roundtrip:
	PYTHONPATH=apps/api $(PYTHON) scripts/manual_recipe_roundtrip.py
	$(PYTHON) scripts/probe_emitted_mcp_commands.py \
...
evidence-backfill:
	PYTHONPATH=apps/api $(PYTHON) scripts/backfill_evidence_store.py
...
verify-top-candidates:
	PYTHONPATH=apps/api $(PYTHON) scripts/verify_top_candidates.py --limit $${LIMIT:-10}
...
# so both `verification_records` and `benchmark_runs` get fresh rows on
# every cron tick. Wired together by sprint-pitch-align Phase 3.
evidence-cron: verify-top-candidates benchmark-cron
...
discovery-verify-cron:
	@set -a; . ./.env 2>/dev/null || true; set +a; \
	  PYTHONPATH=apps/api $(PYTHON) scripts/run_discovery_verify_cron.py \
...
check-evidence-health:
	@set -a; . ./.env 2>/dev/null || true; set +a; \
	  PYTHONPATH=apps/api $(PYTHON) scripts/check_evidence_health.py \
...
backfill-phase5-cells:
	@set -a; . ./.env 2>/dev/null || true; set +a; \
	  PYTHONPATH=apps/api $(PYTHON) scripts/backfill_phase5_cells.py \
...
demand-snapshot:
	@set -a; . ./.env 2>/dev/null || true; set +a; \
	  PYTHONPATH=apps/api $(PYTHON) scripts/export_demand_snapshot.py \
...
audit-baseline-firewall:
	$(PYTHON) scripts/audit_baseline_firewall.py
