...
<span class="muted">We index AI agents, MCP servers, A2A agents, and AI-native services, and rank them with evidence. We hand users honest recipes — and refuse with reasons when no provider is tested and configured. Our benchmark engine and credibility classifier are built; populating them with real, scored cells for discovered agents is the roadmap this raise funds. Built for a future where AI agents themselves call this layer to discover, vet, and orchestrate the agents they need to complete a goal.</span>
...
| **Discovery** | 17 wired scouts → index → normalizer → dedupe → LLM candidate judge → 5-tier verification. Sources: 5 curated manifests (98 catalog seeds across MCP / A2A / AI-agent / web-docs) + 12 remote: 4 MCP-aggregator catalogs (official MCP Registry, MCP Marketplace, Smithery, Glama — combined *upstream* surface ~30,000 servers) + 1 npm package-registry scout (~47K MCP-tagged packages) + 3 GitHub scouts (code search, recently-pushed, awesome-* list scraper) + APIs.guru, HN agent-watcher, Vendor RSS, Moltbook. Diligence caveat: 5 scouts are key-gated (GitHub / Smithery / Moltbook) and silently skip without keys, and semantic recall depends on a real embedder (Ollama/OpenAI) — a keyless, hash-embedder deploy runs far fewer producing scouts than 17. "~30,000 servers" is the upstream catalog surface, not the count we have indexed today |
...
| **Benchmark engine** | **Built and honest, not yet populated with discovered-agent cells.** The harness (runner + field-weighted scorer + per-capability rankings + a conservative credibility classifier) is implemented and tested. What exists today are *baseline* cells: hand-coded wrappers around commodity **APIs** (Razorpay `payment_authorization` exercised live; Resend `email_send` and Firecrawl `web_scraping` exercised via deterministic fixture transports pending live keys). These baselines are reference points only — by design they are firewalled out of the user-facing routing path (`is_benchmark_baseline`). **No discovered MCP/A2A *agent* has been benchmarked end-to-end and persisted into the ranking store yet**, so the credibility classifier currently reports the public leaderboards as `synthetic_only` (see `reports/benchmark-credibility/`). Closing that loop — execute a discovered MCP server against cases, score it, persist it, let the classifier promote it past `synthetic_only` — is the first real "cell" and the primary thing the seed funds. 25 hand-authored cases across 5 capabilities; adversarial cases verify wrappers can't hallucinate ids or content |
| **Live evidence layer** | Public `/health/evidence` + `/evidence/recent-verifications` endpoints back a homepage strip and a "Recent verifications" panel on `/discovery-gaps` and `/open-mcp-opportunities`. Five evidence tables (`benchmark_runs`, `verification_records`, `discovery_run_events`, `capability_demand_events`, `discovery_gap_events`) are populated and queried live when the stack runs against Postgres — counts above the fold are Postgres rows, not slide numbers. Caveat for diligence: today the `benchmark_runs` rows are baseline/synthetic, not discovered-agent cells (see Benchmark engine row) |
...
|---|---|---|---|
| **1 — Plan + Discover + Recipe** *(now → Y1)* | Free discovery, free recipe exports, Pro tier (Clerk auth + Stripe billing + workspaces + saved recipes) shipped as a skeleton today | Developers, operators | Building today — **discovery index + honest-refusal planner + demand-signal surfaces are real**; **benchmark engine + credibility classifier are built but not yet populated with discovered-agent cells** (leaderboards currently labelled `synthetic_only`); **15 hand-curated baseline providers / 26 capabilities / 98 catalog seeds**, **17 wired scouts on a 24h cron**, evidence-health CI guard on `main` |
...
| Homepage live-evidence strip | All visitors | 4 tiles backed by Postgres: total benchmark runs, verifications (7d), scout runs (24h), demand events (24h). Benchmark-run tile is baseline/synthetic today, labelled as such | Free |
...
2. **Build the first real benchmark cell, then grow coverage.** A "cell" is one capability (e.g., `payment_authorization`) scored end-to-end for pass/fail, quality, latency, and cost. Today's cells are *baseline* wrappers around commodity APIs (Razorpay live; Resend + Firecrawl fixture-backed); none is a *discovered agent* yet. The first milestone is to close the discovered-agent loop: execute a discovered MCP server against cases, score it, persist it, and let the credibility classifier promote that capability past `synthetic_only`. Then widen into the recurring commodity sub-tasks — email verification (Hunter is already `capability_verified`), web search, contact enrichment, transactional email, shipping — until ≥ 4 capabilities reach the `developing`/`publishable` band with real (not fixture, not synthetic) runs.
3. **Operate the evidence loop in production.** The cron is shipped — `make benchmark-cron`, `make discovery-verify-cron`, `make evidence-cron`, the `evidence_health` Postgres view, and a GitHub Actions guard that fails `main` when the live counters drop to zero. The 90-day job is to land it on the production host, alert on its failures, and watch the *real-cell* count climb from 0 past 3.
...
|---|---|---|---|
| Index agent supply (17 wired scouts; key-gated + embedder-dependent for full recall) | ✅ | | |
...
| Trust evidence model (5-tier verification ladder, MCP `tools/list` probe, demand signals) | ✅ verification records + 5 evidence tables queried live; CI guard fails `main` on cron regression | | |
| Benchmark evidence model for *discovered agents* | | ✅ engine + credibility classifier built; first real cell is the next milestone — leaderboards are `synthetic_only` until then | |
